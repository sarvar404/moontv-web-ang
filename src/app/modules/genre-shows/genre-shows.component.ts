...
import { NodejsApiService } from 'src/app/services/nodejs-api-service.service';
...
export class GenreShowsComponent implements OnInit{
  ...
  constructor(private route: ActivatedRoute,
    ...
    private nodeJsService: NodejsApiService,
  ...
  chunkArray2<T>(posters: Poster[], size: number): any {
    const results: any = [];
    for (let i = 0; i < posters.length; i += size) {
      const chunk = posters.slice(i, i + size);
      results.push(chunk);
  ...
  loadTvShowPageData(data: { genre: number, order: string, page: number }): void {
    ...
    //  this.posters =[];
    // }
    this.nodeJsService.getTvShowsData(data).subscribe({
      next: (resp: Poster[]) => {
        console.log(resp);
        if (resp.length > 0) {
  ...
  loadMoviePageData(data: { genre: number, order: string, page: number }): void {
    ...
    //  this.posters =[];
    // }
    this.nodeJsService.getMovieData(data).subscribe({
      next: (resp: Poster[]) => {
        console.log(resp);
        if (resp.length > 0) {
          this.posters = [...this.temp2, ...resp];
          this.temp2 = this.posters;
          this.paginatedPosters = this.chunkArray2(this.temp2, 30);
