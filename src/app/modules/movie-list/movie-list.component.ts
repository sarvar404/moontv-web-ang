...
export class MovieListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  selectedGenre: Genere = { id: 0, title: "All Movies", posters: [] };
  switchlang = 'en'
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      if (typeof id === 'undefined' || id == 0) {
        this.selectedGenre = { id: 0, title: "All Movies", posters: [] };
      } else {
  ...
  scroll = (event: any): void => {
    ...
    if (!this.paginationLoading && !this.pausePagination) {
      if (windowBottom >= docHeight) {
        ...
        this.loadMoviePageData({ genre: this.selectedGenre?.id!, order: 'created', page: this.getNextPageNumber()+1  });
      }
    }
  ...
  getGeneres(data: { genre: string }) {
    this._genreService.getGenere(data).subscribe((resp) => {
      // console.log('generesss',res);
      this.genres = [];
      this.genres.push({ id: 0, title: "All Movies", posters: [] });
      resp.sort((a, b) => {
      ...
      resp.forEach(genre => {
        if (this.filterMovieGenres.indexOf(genre.id!) >= 0) {
          console.log(genre.posters?.length)
  ...
  loadMoviePageData(data: { genre: number, order: string, page: number }): void {
    ...
    this._movieService.getMovieData(data).subscribe({
      next: (resp: Poster[]) => {
        console.log(resp);
        if (resp.length > 0) {
          // remove adult content from all movies category
          const filteredMovies = resp.filter((movie: Poster) => {
            return !movie.genres?.some((genre) => genre.id === AppContants.GENRE_ADULT);
          });
          //console.log(JSON.stringify(filteredMovies));
          this.posters = [...this.temp, ...filteredMovies];
          this.temp = this.posters;
          this.paginationMap.set(data.page, this.posters);
          //this.paginatedPosters = this.chunkArray(this.temp, 30);
          this.paginatedPosters.push(filteredMovies);
        }
        else {
